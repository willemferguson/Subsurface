...
struct TemperatureBinner : public IntRangeBinner<TemperatureBinner, IntBin> {
	bool air;
	TemperatureBinner(int bin_size, bool airIn) :
...
struct WaterTemperatureType : TemperatureType {
	...
	std::vector<const StatsBinner *> binners() const override {
		return { &water_temperature_binner2, &water_temperature_binner5,
			 &water_temperature_binner10, &water_temperature_binner20 };
	}
	double toFloat(const dive *d) const override {
...
struct AirTemperatureType : TemperatureType {
	...
	std::vector<const StatsBinner *> binners() const override {
		return { &air_temperature_binner2, &air_temperature_binner5,
			 &air_temperature_binner10, &air_temperature_binner20 };
	}
	double toFloat(const dive *d) const override {
