...
void TabDiveInformation::checkDcSalinityOverWritten()
{
	...
	if (current_dive && dc_value && user_value) {
		if (user_value != dc_value) {
			if (dc_value >=10250 && user_value >=10250)              // Provide for libdivecomputer that defines
				ui->salinityOverWrittenIcon->setVisible(false);  // dc seawater density as 1.025 in
			else                                                     // contrast to Subsurface's value of 1.03
